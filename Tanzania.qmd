---
title: "Tanzania: Alumnae Snapshot"
toc: true
format:
  html:
    theme: cosmo
...
<p style="color:#475569; font-family:Georgia,serif; font-size:1rem; margin-top:-.5rem; margin-bottom:1rem;">
  This snapshot highlights how alumnae from Tanzania navigate key life milestones such as education, marriage, and motherhood, surveyed one and five years after completing the program.
</p>
...
tanz <- df_clean %>% filter(country == "Tanzania")

m_mean  <- function(x) if (all(is.na(x))) NA_real_ else mean(as.numeric(x), na.rm = TRUE)
m_share <- function(x) mean(x, na.rm = TRUE)

marital_raw <- if ("n3_b1" %in% names(tanz)) tanz$n3_b1 else NA_character_
age_marriage_col <- c("n3_b2")
age_marriage_col <- age_marriage_col[age_marriage_col %in% names(tanz)][1]
age_first_marriage <- if (!is.null(age_marriage_col)) tanz[[age_marriage_col]] else NA_real_
has_children_col <- c("n3_b3")
has_children_col <- has_children_col[has_children_col %in% names(tanz)][1]
has_children <- if (!is.null(has_children_col)) tanz[[has_children_col]] else NA
num_children_col <- c("n3_b4")
num_children_col <- num_children_col[num_children_col %in% names(tanz)][1]
num_children <- if (!is.null(num_children_col)) tanz[[num_children_col]] else NA_real_
college_flag <- !is.na(tanz$n2_a5)
enrolled_now <- if ("n2_a2" %in% names(tanz)) tolower(as.character(tanz$n2_a2)) %in% c("1","yes","y","true","t") else NA
...
<div style="position:relative; margin:0 0 1.25rem 0;">
  <img src="assets/tz.jpg" alt="Tanzania alumnae"
...
```{r, echo=FALSE, warning=FALSE, message=FALSE}
# --- Tanzania At a Glance: waffle + year-split slideshow (2023 vs 2024, by cohort) ---
...
tanz <- df_clean %>% filter(country == "Tanzania")
stopifnot("survey_year" %in% names(tanz))

marital_all <- coerce_marital(tanz)
...
coord_equal()+
labs(title="Out of 100 alumnae from Tanzania",
...
years <- sort(unique(tanz$survey_year))
...
summary_tbl <- tibble(
Metric=c(
"Number of Tanzania Alumnae Surveyed",
"Average Age",
"% Married",
"% Never married",
...
glue("
  <div class='slide' role='group' aria-label='Table for {title_year}' data-year='{title_year}'>
    <div style='display:flex;justify-content:space-between;align-items:center;margin:0 0 .5rem 0;'>
      <h3 style='margin:0;font-family:Georgia,serif;'>Tanzania At a Glance: {title_year}</h3>
    </div>
    {tbl_html}
...
df_year <- tanz %>% filter(survey_year==yr)
