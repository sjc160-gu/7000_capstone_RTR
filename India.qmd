...
<p style="color:#475569; font-family:Georgia,serif; font-size:1rem; margin-top:-.5rem; margin-bottom:1rem;">
  This snapshot highlights how alumnae from India navigate key life milestones such as education, marriage, and motherhood, who were surveyed 1 year and 5 years after completing the program.
</p>
...
ind <- df_clean %>% filter(country == "India")

m_mean  <- function(x) if (all(is.na(x))) NA_real_ else mean(as.numeric(x), na.rm = TRUE)
m_share <- function(x) mean(x, na.rm = TRUE)

marital_raw <- if ("n3_b1" %in% names(ind)) ind$n3_b1 else NA_character_
age_marriage_col <- c("n3_b2","age_first_marriage","b2_age_first_marriage")
age_marriage_col <- age_marriage_col[age_marriage_col %in% names(ind)][1]
age_first_marriage <- if (!is.null(age_marriage_col)) ind[[age_marriage_col]] else NA_real_
has_children_col <- c("n3_b3","has_children","b3_has_children")
has_children_col <- has_children_col[has_children_col %in% names(ind)][1]
has_children <- if (!is.null(has_children_col)) ind[[has_children_col]] else NA
num_children_col <- c("n3_b4","children_number","b4_children_count")
num_children_col <- num_children_col[num_children_col %in% names(ind)][1]
num_children <- if (!is.null(num_children_col)) ind[[num_children_col]] else NA_real_
college_flag <- !is.na(ind$n2_a5)
enrolled_now <- if ("n2_a2" %in% names(ind)) tolower(as.character(ind$n2_a2)) %in% c("1","yes","y","true","t") else NA
...
<div style="position:relative; margin:0 0 1.25rem 0;">
  <img src="assets/in.jpg" alt="India alumnae"
...
```{r, echo=FALSE, warning=FALSE, message=FALSE}
# --- India At a Glance: waffle + year-split slideshow (2023 vs 2024, by cohort) ---
# minimal deps

# ---- helpers ----
m_mean  <- function(x) if (all(is.na(x))) NA_real_ else mean(suppressWarnings(as.numeric(x)), na.rm = TRUE)
...
# College (A1/A2): ever enrolled or currently enrolled in higher education
coerce_college <- function(df) {
  if (!"n2_a5" %in% names(df)) return(rep(NA_real_, nrow(df)))
  v <- df[["n2_a5"]]
  # Treat NA as 0, anything non-missing as 1
  return(ifelse(is.na(v), 0, 1))
}

# Children
coerce_children_count <- function(df) {
  count_cand <- get_first_col(df, c("n3_b4", "n3_c2","num_children","children_count","child_count"))
  if (!is.null(count_cand)) {
    v <- suppressWarnings(as.numeric(df[[count_cand]]))
    return(v)
  }
  has_cand <- get_first_col(df, c("n3_b3", "n3_c1","has_children","children","any_children"))
  if (!is.null(has_cand)) {
    v <- df[[has_cand]]
    if (is.logical(v)) return(ifelse(v, 1, 0))
    if (is.numeric(v)) return(v)
    if (is.character(v) || is.factor(v))
      return(ifelse(tolower(as.character(v)) %in% c("yes","y","true","1"), 1, 0))
  }
...
coerce_marital <- function(df) {
  cand <- get_first_col(df, c("n3_b1","marital_status","married"))
...
coerce_age_first_marriage <- function(df) {
  cand <- get_first_col(df, c("n3_b2","age_first_marriage","b2_age_first_marriage"))
  if (is.null(cand)) return(rep(NA_real_, nrow(df)))
  suppressWarnings(as.numeric(df[[cand]]))
}
...
compute_metrics <- function(df_slice) {
  ...
  list(
    ...
    share_has_children = if (all(is.na(has_children))) NA_real_ else m_share(has_children),
    avg_children_among_mothers = if (all(is.na(kids_count)) || all(kids_count %in% c(NA, 0))) NA_real_ else m_mean(kids_count[kids_count > 0]),
    share_college = m_share(college_tf)
  )
}

fmt_num <- function(x, digits = 1) {
  ifelse(is.na(x), "—", round(x, digits))
}

fmt_pct <- function(x) {
  ifelse(is.na(x), "—", percent(x, 1))
}

# =========================
# Data slice: India only
# =========================
ind <- df_clean %>% filter(country == "India")
stopifnot("survey_year" %in% names(ind))

# ----- Waffle chart -----
marital_all <- coerce_marital(ind)
...
palette <- c("Married" = "#007AA4",
             "Never married" = "#A3C754",
             "Other/NA" = "#B3B3B3")
...
ggplot(waffle_df, aes(x, y, fill = group)) +
  ...
  coord_equal() +
  labs(
    title = "Out of 100 alumnae from India",
...
# ----- Year-split tables (2023 vs 2024), with 1-year vs 5-year columns -----
years <- sort(unique(ind$survey_year))
...
make_table_html <- function(df_year, title_year) {
  ...
  summary_tbl <- tibble(
    Metric = c(
      "Number of Indian Alumnae Surveyed",
      "Average Age",
      "% Married",
      "% Never married",
  ...
  glue("
    <div class='slide' role='group' aria-label='Table for {title_year}' data-year='{title_year}'>
      <div style='display:flex;justify-content:space-between;align-items:center;margin:0 0 .5rem 0;'>
        <h3 style='margin:0;font-family:Georgia,serif;'>India At a Glance: {title_year}</h3>
      </div>
      {tbl_html}
...
slides <- map_chr(years, function(yr) {
  df_year <- ind %>% filter(survey_year == yr)
...
<style>
  ...
  #{widget_id} .btn {{
    border:1px solid #E5E7EB; padding:.4rem .7rem; border-radius:10px; background:#fff; cursor:pointer;
    box-shadow:0 2px 8px rgba(0,0,0,.04);
  }}
  #{widget_id} .btn:disabled {{ opacity:.5; cursor:not-allowed; }}
  #{widget_id} .year-pill {{
    font-size:.9rem; padding:.2rem .55rem; border-radius:999px; border:1px solid #E5E7EB; background:#F9FAFB;
  }}
</style>
")
...
<div style="margin-top:.5rem; padding:1.2rem 1.4rem; background:#F8FAFC;
            border:1px solid #E5E7EB; border-radius:12px;">
  <p style="color:#334155; font-family:Georgia,serif; font-size:1rem; line-height:1.6;">
    The India snapshot reveals distinct transitions between alumnae cohorts. Among the 
    <strong style="font-size:1.05rem; color:#0F3B4A;">1-year alumnae</strong>, the average age is around 20, 
    with about <strong style="font-size:1.05rem; color:#0F3B4A;">one-in-four married</strong> and 
    <strong style="font-size:1.05rem; color:#0F3B4A;">fewer than one-in-ten</strong> having children. Most are continuing their studies, roughly <strong style="font-size:1.05rem; color:#0F3B4A;">two-thirds</strong> have pursued
    post-secondary education, showing that higher education remains a strong priority early in adulthood.
    <br><br>
    For the <strong style="font-size:1.05rem; color:#0F3B4A;">5-year alumnae</strong>, the picture shifts: 
    about <strong style="font-size:1.05rem; color:#0F3B4A;">half are married</strong>, 
    <strong style="font-size:1.05rem; color:#0F3B4A;">two-in-five have children</strong>, and 
    around <strong style="font-size:1.05rem; color:#0F3B4A;">one-in-four</strong> have continued to higher education.  

    While marriage and motherhood become more common with time, educational aspirations persist.
  </p>
</div>
