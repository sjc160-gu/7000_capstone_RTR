---
title: "Cambodia: Alumnae Snapshot"
toc: true
format:
  html:
    theme: cosmo
...
cam <- df_clean %>% filter(country == "Cambodia")

m_mean  <- function(x) if (all(is.na(x))) NA_real_ else mean(as.numeric(x), na.rm = TRUE)
m_share <- function(x) mean(x, na.rm = TRUE)

marital_raw <- if ("n3_b1" %in% names(cam)) cam$n3_b1 else NA_character_
age_marriage_col <- c("n3_b2")
age_marriage_col <- age_marriage_col[age_marriage_col %in% names(cam)][1]
age_first_marriage <- if (!is.null(age_marriage_col)) cam[[age_marriage_col]] else NA_real_
has_children_col <- c("n3_b3")
has_children_col <- has_children_col[has_children_col %in% names(cam)][1]
has_children <- if (!is.null(has_children_col)) cam[[has_children_col]] else NA
num_children_col <- c("n3_b4")
num_children_col <- num_children_col[num_children_col %in% names(cam)][1]
num_children <- if (!is.null(num_children_col)) cam[[num_children_col]] else NA_real_
college_flag <- !is.na(cam$n2_a5)
enrolled_now <- if ("n2_a2" %in% names(cam)) tolower(as.character(cam$n2_a2)) %in% c("1","yes","y","true","t") else NA
...
<div style="position:relative; margin:0 0 1.25rem 0;">
  <img src="assets/kh.jpg" alt="Cambodia alumnae"
...
```{r, echo=FALSE, warning=FALSE, message=FALSE}
# --- Cambodia At a Glance: waffle + year-split slideshow (2023 vs 2024, by cohort) ---
...
stopifnot("survey_year" %in% names(cam))

marital_all <- coerce_marital(cam)
...
coord_equal()+
labs(title="Out of 100 alumnae from Cambodia",
...
years <- sort(unique(cam$survey_year))
...
glue("
  <div class='slide' role='group' aria-label='Table for {title_year}' data-year='{title_year}'>
    <div style='display:flex;justify-content:space-between;align-items:center;margin:0 0 .5rem 0;'>
      <h3 style='margin:0;font-family:Georgia,serif;'>Cambodia At a Glance: {title_year}</h3>
    </div>
    {tbl_html}
...
df_year <- cam %>% filter(survey_year==yr)
